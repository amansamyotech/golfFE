...
export const addStaff = async (payload: FormData | StaffPayload) => {
...
export const updateStaff = async (id: string, payload: FormData | StaffPayload) => {
...
export const getStaffByEmail = async (email: string) => {
    const url = `${api_urls.baseUrl}${api_urls.staff.getByEmail}/${encodeURIComponent(email)}`;
    const response = await getData(url) as { status?: number; data?: unknown };
