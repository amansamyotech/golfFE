...
// Extend all native input props, then add your custom ones
...
const Input: FC<InputProps> = ({
  ...
  hint,
  disabled = false,
  ...rest // includes name, value, onChange, maxLength, etc.
}) => {
  // Determine input styles dynamically
  ...
  if (disabled) {
    ...
    inputClasses += ` text-error-800 border-error-500 focus:ring-3 
    focus:ring-error-500/10 dark:text-error-400 dark:border-error-500`;
  } else if (success) {
  ...
  return (
    <div className="relative">
      <input
        ...
        disabled={disabled}
        className={inputClasses}
        {...rest} // ✅ spreads all remaining props (name, value, onChange, maxLength, etc.)
      />
